/*
 * define.h
 *
 *  Created on: Feb 6, 2012
 *      Author: pom
 *
 *     LEN HLUPACI SI NEPISU COMMENTY A POTOM PLACU ZE NEVEDIA CO TYM CHCELI POVEDAT!!!:'(
 *
 *
 */

#ifndef DEFINE_H_
#define DEFINE_H_

//#define FORWII
#define O_TYPE int
#define O_SYMBOL_COUNT 4		//	kolko znakov ma abeceda pohybu (do ktorej premieta quantizer)
#define HMM_PROB_TYPE double
#define HMM_STATE_COUNT 8
#define VERBOSITY 0			//	cim vyssia tym viac povie
#define r_trans 0.0000000			//	pravdepodobnost ze ???
#define r_emiss 0.00000001			//	pravdepodobnost ze ???
#define WINDOW_SIZE 3			//	dlzka rozpoznavaneho gesta

#endif /* DEFINE_H_ */
